...
import { Trophy, ExternalLink, ThumbsUp, Share, Download, FileCode, Loader2 } from "lucide-react";
...
export default function SummaryView({ data, goHome, roomId }: SummaryViewProps) {
  ...
  const room = roomData?.rooms?.[0] as any;
  const roomPlayers: any[] = room?.players ?? [];
  const roomQueueItems: any[] = room?.queueItems ?? [];

  const roomCode = data ? data.roomCode : (room?.code ?? "");

  const playedSongs: SummarySong[] = data
    ? data.songs
    : roomQueueItems
        .filter((q: any) => q.status === "PLAYED" || q.status === "SKIPPED")
        .sort((a: any, b: any) => a.createdAt - b.createdAt)
        .map((q: any) => ({
          id: q.id,
          video_id: q.videoId,
          video_title: q.videoTitle || "Unknown Track",
          player_nickname:
            roomPlayers.find((p: any) => p.id === q.playerId)?.nickname || "Unknown",
              ...
              return (
                ...
                >
                    ...
                    <div className="truncate">
                      <h3 className="font-bold text-lg flex items-center space-x-2">
                        <span className="truncate">
                          {song.video_title || `${t("unknownTrack")} (${song.video_id})`}
                        </span>
                        {song.status === "SKIPPED" && (
                          <span className="text-[10px] uppercase tracking-widest bg-red-500/20 text-red-500 px-2 py-0.5 rounded-full font-bold">
                            {t("skipped")}
                          </span>
                        )}
                      </h3>
                  ...
                  <div className="flex items-center space-x-4 shrink-0">
                    {score !== null && (
                      <div
                        className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg border ${
                          score > 75
