...
export default function GameView({ room }: GameViewProps) {
  ...
  useEffect(() => {
    ...
    // Fallback: Force ready after 5 seconds if YouTube API hangs
    const timer = setTimeout(() => {
        setIsPlayerReady((prev) => {
            if (!prev) {
                console.warn("Host Player: Force Ready Timeout Triggered");
                return true;
            }
            return prev;
        });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Only run if playing and we have a start time
      ...
      // Trigger next song when timer hits 0
  ...
  const handleNextSong = async () => {
    ...
    if (pendingItems.length === 0) {
        console.log("Queue empty. Pausing.");
        txs.push(
            db.tx.rooms[currentRoom.id].update({
        ...
        console.log("Playing next:", nextItem.video_id);
        ...
        <div className="absolute inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-8 animate-in fade-in zoom-in duration-200">
            <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl max-w-lg w-full space-y-8 relative">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-neutral-400">Customize the party rules.</p>
                </div>

                <div className="space-y-4">
                    <label className="text-xs font-bold uppercase tracking-widest text-neutral-500">Timer Duration</label>
                ...
                <div className="space-y-4 pt-4 border-t border-neutral-800">
                    <label className="text-xs font-bold uppercase tracking-widest text-neutral-500">Add Time to Current Song</label>
        ...
        <div className="absolute inset-0 z-[100] bg-black/90 backdrop-blur-sm flex items-center justify-center p-8 animate-in fade-in zoom-in duration-200">
            <div className="bg-neutral-900 border border-neutral-800 p-8 rounded-3xl max-w-2xl w-full space-y-8 relative max-h-[80vh] flex flex-col">
                ...
                <div className="space-y-2 shrink-0">
                    ...
                    <p className="text-neutral-400">Kick players or grant VIP status.</p>
                </div>
                        ...
                        <div key={player.id} className="flex items-center justify-between bg-neutral-800/50 p-4 rounded-2xl border border-neutral-700/50">
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-neutral-500">{player.is_vip ? "VIP DJ" : "Guest"}</p>
                                </div>
                            </div>
    ...
    return (
      <div className="w-full h-full flex flex-col animate-in fade-in duration-1000 relative overflow-hidden">
        <div className="flex-1 flex flex-col items-center justify-center space-y-6 animate-pulse text-center">
            ...
            <div className="space-y-2">
                <h2 className="text-3xl font-bold italic text-neutral-400">Waiting for tracks...</h2>
                <p className="text-neutral-600">The queue is empty. Add some fire on your phones!</p>
            </div>
        </div>
            ...
            <div className="flex items-center space-x-8 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setShowPlayers(true)}>
                <div className="flex -space-x-3">
                    {room.players.map((p, i) => (
                        <div 
                            key={p.id} 
                            className={`w-10 h-10 rounded-full border-2 bg-neutral-800 flex items-center justify-center font-bold text-xs ${p.is_vip ? 'border-yellow-500 text-yellow-500' : 'border-neutral-950 text-white'}`}
                ...
                <div className="text-neutral-500 text-sm font-medium">
                    {room.players.length} DJs Online
                </div>
            </div>
    ...
    <div className="w-full h-full flex flex-col animate-in fade-in duration-1000 relative overflow-hidden">
      ...
      <div className="flex justify-between items-center px-4 py-4 shrink-0 h-20">
        <div className="space-y-0.5">
            ...
            <h1 className="text-3xl font-black max-w-xl truncate leading-none">
                {activePlayer ? `${activePlayer.nickname}'s Pick` : "Next Up"}
            </h1>
        </div>

        <div className="flex flex-col items-end">
            <div className={
                `flex items-center space-x-3 px-5 py-1.5 rounded-2xl border-2 font-mono text-3xl font-black transition-colors ${
                    timeLeft < 20 ? "border-red-500 text-red-500 animate-pulse" : "border-neutral-800 text-white"
                }`
            }>
                <Clock size={20} />
      ...
      <div className="flex-1 min-h-0 w-full relative bg-black rounded-3xl overflow-hidden border border-neutral-800 shadow-[0_0_100px_rgba(79,70,229,0.1)]">
        ...
        <iframe
            ...
            src={`https://www.youtube.com/embed/${room.current_video_id}?autoplay=1&start=${room.current_start_time}&controls=0&modestbranding=1&rel=0`}
      ...
      <div className="flex items-center justify-between px-4 py-4 shrink-0 h-20">
        ...
        <div className="flex items-center space-x-6">
            ...
            >
                <span className="font-bold uppercase tracking-widest text-xs">Skip Song</span>
